...
```bash
lambda-pytorch$ sam build --use-container
```
...
```bash
lambda-pytorch$ sam logs -n StockCheckerFunction --stack-name lambda-pytorch --tail
```
...
```bash
lambda-pytorch$ pip install -r tests/requirements.txt --user
# unit test
lambda-pytorch$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
lambda-pytorch$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name lambda-pytorch
```
